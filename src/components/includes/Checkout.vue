    ...
    <section id="cart_items">
      <div class="container">
        ...
        <div class="row mt-4" style="margin-left: 3px; margin-top: 3px">
          <div>
          <label><input type="radio" name="payment_mode" /> Cash on delivery </label>&nbsp;  
         <label><input type="radio" name="payment_mode"  /> Paypal </label>&nbsp;  

          </div>
        </div>

        <div class="step-one">
          <h2 class="heading"> Coupon & discounts</h2>
        </div>
        <div class="col-sm-6">
      ...
      items.forEach((item) => {
        ...
        UserOrderDetails(obj).then((res) => {
          if (res) {
            ...
            this.$swal("Order placed","Thankyou for shopping Have a nice day!!",'success')
            localStorage.removeItem('discount')
            this.check=JSON.parse(localStorage.getItem("myCheckout")); 
            let arr = JSON.stringify(this.check);
            JSON.parse(localStorage.setItem("orders",arr));
